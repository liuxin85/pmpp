...
// Constant memory declaration for the convolution kernel
__constant__ float constKernel[KERNEL_SIZE * KERNEL_SIZE];

// CUDA kernel for 2D convolution using constant memory
        ...
        for (int ch = 0; ch < channels; ch++) {
            ...
            for (int ky = -KERNEL_SIZE/2; ky <= KERNEL_SIZE/2; ky++) {
                for (int kx = -KERNEL_SIZE/2; kx <= KERNEL_SIZE/2; kx++) {
                    ...
                    // Get kernel value from constant memory
                    int kernelIdx = (ky + KERNEL_SIZE/2) * KERNEL_SIZE + (kx + KERNEL_SIZE/2);
                    float kernelValue = constKernel[kernelIdx];
...
// Host function to perform convolution with constant memory
void convolution2D(float* h_input, float* h_output, float* h_kernel, 
    ...
    cudaMemcpyToSymbol(constKernel, h_kernel, 
    ...
    printf("GPU Execution Time (with constant memory): %.3f ms\n", milliseconds);
...
int main() {
    ...
    // Perform convolution on GPU with constant memory
